...
$( function() {
	...
	// Change cs_container padding-top
	setCSCPadding();
...
// On Click Actions
	...
	$("body").on("click", "#user_car", function (e) {
		...
		var lastPartID ="";
		// Empty $("#part_list_container")
		$("#part_list_container").html("");
		...
		$.post(site_root+'app/ajax-controllers/partStoreAjax.php', {
			...
			data['parts'].forEach(function (part) {
				...
				partListHTML = 	'<div id="partContainer" data-id="' + part['pt_id'] + '" data-storeid="' + storeID + '" style="cursor: pointer;">'+
									'<div id="partName" class="fadeOut" style="text-align: center; font-family: rootbear; font-size: 32px; background-color: rgba(0, 0, 0, .8); color: #fff; margin: 15px auto; border-radius: 5px; -webkit-box-shadow: 5px 9px 25px 0px rgba(0,0,0,0.75); -moz-box-shadow: 5px 9px 25px 0px rgba(0,0,0,0.75); box-shadow: 5px 9px 25px 0px rgba(0,0,0,0.75);">'+
								...
								'</div>'+
								'<div id="partDescription_' + part['pt_id'] + '" data-id="' + part['pt_id'] + '" style="text-align: center; display: none; margin: 20px auto; font-family: calibriB; font-size: 18px;">'+
									part['pt_description']+
								'</div>'+
				...
				$("#part_list_container").append(partListHTML);
				$("#hp").html(hp);
				$("#tq").html(tq);
				$("#weight").html(weight);
	...
	$("body").on("click", "#buyNow", function (e) {
		...
		$.post(site_root+'app/ajax-controllers/partStoreAjax.php', {
			...
			alert("error:" + data['error'] + "; e-msg: " + data['e_msg'] + "; Bought: " + data['bought']);
		});
	});
		...
		data['parts_available'].forEach(function (part) {
			...
			part_template =	'<div id="selected_' + count + '" class="select_highlight" style="display: none;"></div>' +
							'<div id="user_car" class="ic_container" data-id="' + count + '" data-type="' + part['pt_type'] + '" data-storeid="' + storeID + '" style="background-color: #fff;">' +
								'<img id="uc_img" src="' + data['image_root'] + 'parts-store/' + part['pt_type'] + '-icon.png" style="max-height: 150px; "/>' +
							'</div>'+
